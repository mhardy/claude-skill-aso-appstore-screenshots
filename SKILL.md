...
- **Primary — Feature zoom-out (only when relevant)**: If there is an obvious, visually compelling entire UI panel or grouped section on the app screen that directly reinforces the benefit headline, make it "pop out" from the device frame via `--breakout`. The panel stays at the same vertical position and orientation as where it appears on the app screen — the script doesn't rotate or angle it. It extends beyond both left and right edges of the device frame (tune `zoom` until it clearly overlaps the phone bezel on both sides) and gets a soft drop shadow so it reads as floating above the device. The box you pass must crop a complete card/section — not an individual button, icon, or small element. If no panel clearly relates to the headline, skip the breakout entirely.
  ...
  **Fit the box to the card's real pixel bounds — don't eyeball generous margin around it.** If the source UI element already has its own rounded corners baked into the screenshot (most cards do), any margin between your `box` edges and the card's actual edges bakes a strip of the *source's own background* into the crop. `--breakout`'s mask only rounds the four corners, not the flat edges, so this shows up as a gap between the white ring and the photo along the ENTIRE flat edge, not just at the corners — confirmed directly: a box with ~14px of left-edge margin produced an 11px black gap at every single row along that edge, not a corner-only artifact. Find the card's true edges before choosing the box, e.g.:
  ```python
  # scan a flat (non-corner) row/column for where the card's real edge is
  is_bg = lambda p: p[0] < 8 and p[1] < 8 and p[2] < 8
  x = 0
  while is_bg(px[x, y]): x += 1   # x is now the card's true left edge
  ```
  Do this on all four sides (a flat mid-edge row/column, not near a corner) and set `box` to those exact values. Only after the box is tight should you look at the corners: if a small gap remains right at the corner tip (not along the flat edge), *that* is a genuine radius mismatch — the source card's own corner rounding is baked into its pixels at some radius, and `--breakout`'s mask needs `corner` (optional key, default 36) raised to at least match it at final-canvas scale (`corner * scale * zoom` — `scale` comes from `screen_rect`, printed by the `device_y`/`screen_rect` snippet elsewhere in this doc). Verify with the same per-pixel scan, not by eye — a few-px gap is easy to miss at normal zoom and easy to mistake for a corner-radius problem when it's actually a box-margin problem (check a flat edge first, always — it's the more common cause).
